...
namespace DensitySupress.Client
{
    public class Main : BaseScript
    {
        ...
        {
            World.GetAllVehicles().ToList<Vehicle>().ForEach(async delegate (Vehicle
            v)
            {
                if (this._modelsToSuppress.Any((string m) => API.GetHashKey(m) ==
                    v.Model.Hash) && Entity.Exists(v) && !v.PreviouslyOwnedByPlayer)
                {
